...
class GnuplotCanvas(FigureCanvas):
    def __init__(self, MainWindow, x_len:int, y_range:List) -> None:
        ...
        # List of coordinates f_strat_ch_final
        self.y_list_f_strat_ch_final = [0] * x_len

        # Store figures axes
        self.ax = self.figure.subplots()
        return

    def updateCanvas(self, f_C, f_C_corr, avg_payoff, f_strat_ch, f_strat_ch_final) -> None:
        self.ax.clear()

        self.y_list_f_C.append(f_C)
        self.y_list_f_C = self.y_list_f_C[-self.x_len:]
        self.ax.plot(self.x_list, self.y_list_f_C, label = "freq of 1s", color="blue")
    
        self.y_list_f_C_corr.append(f_C_corr)
        self.y_list_f_C_corr = self.y_list_f_C_corr[-self.x_len:]
        self.ax.plot(self.x_list, self.y_list_f_C_corr, label = "freq of 1s correct", color="red")

        self.y_list_avg_payoff.append(avg_payoff)
        self.y_list_avg_payoff = self.y_list_avg_payoff[-self.x_len:]
        self.ax.plot(self.x_list, self.y_list_avg_payoff, label = "avg payoff", color="orange")

        self.y_list_f_strat_ch.append(f_strat_ch)
        self.y_list_f_strat_ch = self.y_list_f_strat_ch[-self.x_len:]
        self.ax.plot(self.x_list, self.y_list_f_strat_ch, label = "freq of strat change", color="yellow")

        self.y_list_f_strat_ch_final.append(f_strat_ch_final)
        self.y_list_f_strat_ch_final = self.y_list_f_strat_ch_final[-self.x_len:]
        self.ax.plot(self.x_list, self.y_list_f_strat_ch_final, label = "freq of strat change final", color="green")
